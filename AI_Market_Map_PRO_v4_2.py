...
# ==========================================
# 📱 모바일 최적화를 위한 커스텀 CSS 주입
# ==========================================
st.markdown("""
<style>
/* 기본 폰트 최적화 및 모바일 기기에서 찌그러짐 방지 */
...
st.caption("🔥 Technical Ranking Screening · Up to 300 Stocks · Mobile Optimized")

with st.sidebar:
    st.header("⚙️ Analysis Settings")
    st.markdown("**🔍 Technical Ranking Screening**")
    
    pool_size = st.slider("Initial Scan Pool (Top US Caps)", 50, 300, 150, 50)
...
with st.form("portfolio_form_us"):
    ...
    st.caption("Use exact Tickers (e.g., AAPL). Click cells to edit.")
...
if st.session_state.get("analysis_complete_us"):
    ...
    with t1:
        ...
        fig.update_layout(height=600, margin=dict(t=5,l=5,r=5,b=5), coloraxis_showscale=True)
        fig.update_traces(
            textinfo="label",
            hovertemplate=(
        ...
        st.caption(f"📌 Base Median Score: {median_score:.1f} pts")
    ...
    with t4:
        ...
        # 모바일 최적화: 5열 및 4열 배치를 2열/3열, 2열/2열로 분리
