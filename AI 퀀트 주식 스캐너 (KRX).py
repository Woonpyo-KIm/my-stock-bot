...
# 앱 이름 변경 적용
st.set_page_config(page_title="AI 퀀트 주식 스캐너 (KRX)", page_icon="🗺️", layout="wide")
...
def analyze_krx(code, name, sector_name="💡 기타 우량주", marcap=0, volume=0):
    ...
    # 조건 A: 볼린저 밴드 상단 돌파 여부
    is_bb_upper = (cur > cur_upper) if pd.notna(cur_upper) else False

    # 조건 B: 당일 거래량이 20일 평균 거래량의 2배(200%) 이상인지 확인 (수급 필터)
    if "Volume" in x.columns and len(x["Volume"]) >= 20:
        vol_series = pd.to_numeric(x["Volume"], errors="coerce").dropna()
        cur_vol = float(vol_series.iloc[-1])
        ma20_vol = float(vol_series.rolling(20).mean().iloc[-1])
        is_vol_surge = (cur_vol >= ma20_vol * 2.0) if (pd.notna(ma20_vol) and ma20_vol > 0) else False
    else:
        is_vol_surge = False

    # 조건 C: 60일 이동평균선 위에 있는지 확인 (대세 상승 필터)
    ma60_series = c.rolling(60).mean()
    cur_ma60 = ma60_series.iloc[-1] if len(ma60_series) >= 60 else np.nan
    is_above_ma60 = (cur > cur_ma60) if pd.notna(cur_ma60) else False

    # 🛡️ 스마트 필터 최종 검증 (3가지 조건 교집합)
    is_smart_breakout = is_bb_upper and is_vol_surge and is_above_ma60
    ...
    trend = (35 if cur > cur_ma20 else 0) + (35 if pd.notna(cur_ma60) and cur > cur_ma60 else 0) + (30 if pd.notna(ma120) and cur > ma120 else 0)
    ...
    # 행동 지침 결정
    if is_smart_breakout: 
        action = "🚀 스마트 BB 돌파 (강력매수)"
    elif is_bb_upper:
        action = "⚠️ BB 돌파 (거래량/60일선 미달 주의)"
    elif score >= 80: 
        action = "🟢 강력 매수"
    elif score >= 70: 
        action = "🟢 매수/보유"
    ...
    return {
        **base, "Current Price": cur, "1Y Return": round(r1, 2), "3M Return": round(r3, 2), 
        ...
        "Action": action, "BB Breakout": is_smart_breakout, "Max Drawdown": round(dd, 2)
    }
...
# ==========================================
# 앱 이름 변경 적용
st.markdown("<h1 style='text-align:center'>🗺️ AI 퀀트 주식 스캐너 (KRX)</h1>", unsafe_allow_html=True)
st.caption("🔥 스마트 볼린저 밴드(수급+60일선) 돌파 로직 탑재 · 시가총액/점수 기반 트리맵 · 한국 증시 최적화")
...
with st.expander("💼 내 보유 포트폴리오 입력 (클릭하여 열기/접기)", expanded=is_expanded):
    ...
    with st.form("portfolio_form_krx"):
        ...
        # 버튼 이름 변경
        run = st.form_submit_button("🗺️ 저장 및 분석 시작", use_container_width=True, type="primary")
...
if st.session_state.get("analysis_complete_krx"):
    ...
    with t1:
        ...
        st.caption("📌 마크: 내 보유 종목 | 🚀 마크: 스마트 볼린저 밴드 돌파 통과 | 색상: 1개월 수익률 (0% 회색, 상승 빨강, 하락 파랑)")
    ...
    with t4:
        ...
        q = q.rename(columns={
            ...
            "BB Breakout": "스마트 BB 돌파", "Current Price": "현재가", "1Y Return": "1년 수익률", 
    ...
    # 안내 문구 변경
    st.info("👆 위 포트폴리오 입력칸에 종목을 입력하고 **저장 및 분석 시작** 버튼을 누르세요.")