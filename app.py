...
st.markdown('<div class="main-title">🗺️ AI MARKET MAP</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">한국 증시 섹터 흐름 · 종목 모멘텀 · 보유종목 분석</div>', unsafe_allow_html=True)
...
def load_universe():
    ...
    for col in required:
        if col not in df.columns:
            raise ValueError(f"FinanceDataReader 데이터에 {col} 컬럼이 없습니다.")
    ...
    candidates = df[df["Marcap"] >= 1_000_000_000_000].head(100)
...
def analyze_stock(stock_code, stock_name, marcap=0, volume=0):
    ...
    if df.empty or "Close" not in df.columns:
        return {
            "종목명": stock_name, "종목코드": stock_code, "섹터": classify_sector(stock_name),
            "시가총액": marcap, "거래량": volume, "현재가": 0, "5년수익률": 0, "1년수익률": 0,
            "3개월수익률": 0, "거래량모멘텀": 0, "변동성": 0, "추세점수": 0, "점수": 0, "차트": pd.DataFrame()
        }
    ...
    if len(close) < 20:
        return {
            "종목명": stock_name, "종목코드": stock_code, "섹터": classify_sector(stock_name),
            "시가총액": marcap, "거래량": volume, "현재가": close.iloc[-1] if len(close) else 0,
            "5년수익률": 0, "1년수익률": 0, "3개월수익률": 0, "거래량모멘텀": 0, "변동성": 0,
            "추세점수": 0, "점수": 0, "차트": df.tail(70)
        }
    ...
    def calc_return(days):
        ...
        return ((current / base) - 1) * 100 if base > 0 else 0
    ...
    if "Volume" in df.columns:
        ...
        if len(volume_series) >= 40:
            recent_avg = volume_series.tail(20).mean()
            previous_avg = volume_series.iloc[-40:-20].mean()
            if previous_avg > 0:
                volume_momentum = ((recent_avg / previous_avg) - 1) * 100

    daily_return = close.pct_change()
    volatility = daily_return.tail(60).std() * np.sqrt(252) * 100
    ...
    trend_score = 0
    if current > ma20: trend_score += 35
    if current > ma60: trend_score += 35
    if current > ma120: trend_score += 30

    score = 50
    if return_5y >= 100: score += 15
    elif return_5y >= 50: score += 10
    elif return_5y > 0: score += 5
    elif return_5y < -30: score -= 10

    if return_1y >= 30: score += 15
    elif return_1y >= 15: score += 10
    elif return_1y > 0: score += 5
    elif return_1y < -20: score -= 10

    if return_3m >= 20: score += 10
    elif return_3m >= 5: score += 5
    elif return_3m < -15: score -= 10

    if volume_momentum >= 50: score += 10
    elif volume_momentum >= 20: score += 5
    elif volume_momentum < -30: score -= 5

    if trend_score >= 80: score += 10
    elif trend_score >= 60: score += 5
    elif trend_score < 35: score -= 5

    if volatility > 80: score -= 5

    score = int(max(0, min(100, score)))

    return {
        "종목명": stock_name, "종목코드": stock_code, "섹터": classify_sector(stock_name),
        "시가총액": marcap, "거래량": volume, "현재가": current, "5년수익률": round(return_5y, 2),
        "1년수익률": round(return_1y, 2), "3개월수익률": round(return_3m, 2),
        "거래량모멘텀": round(volume_momentum, 2), "변동성": round(volatility, 2),
        "추세점수": trend_score, "점수": score, "차트": df.tail(70)
    }
        ...
        if response.status_code == 200:
            ...
            for item in root.findall(".//item"):
                ...
                if title is not None and title.text:
                    text = title.text.replace(" - Yahoo Finance", "").replace(" - Naver", "").strip()
                    if text not in news:
                        news.append(text)
                if len(news) >= limit:
                    break
...
with st.sidebar:
    st.header("⚙️ 분석 설정")
    market_size = st.slider("분석 종목 수", min_value=20, max_value=50, value=40, step=5)
    min_market_cap = st.number_input("최소 시가총액 (조원)", min_value=0.0, max_value=100.0, value=1.0, step=1.0)
    
    st.divider()
    st.markdown("### 📊 점수 기준")
    st.write("- **100점 만점**: 장기 추세, 1년/3개월 수익률, 거래량 변화, 이동평균 추세, 변동성 종합 산출")
...
run_analysis = st.button("🗺️ 시장 지도 분석 시작", use_container_width=True, type="primary")
...
if run_analysis:
    try:
        ...
        filtered_candidates = candidates[candidates["Marcap"] >= min_cap].head(market_size)
        ...
        for stock_name in my_stocks:
            match = krx[krx["Name"].str.strip() == stock_name]
            if match.empty:
                continue
            row = match.iloc[0]
            code = row["Code"]
        ...
        result_df = pd.DataFrame(all_results).sort_values("점수", ascending=False).reset_index(drop=True)
...
if st.session_state.get("analysis_complete", False):
    ...
    with st.container(border=True):
        ...
        c4.metric("1년 수익률", f"{top_pick['1년수익률']:.1f}%")
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🗺️ 시장 트리맵", "🔥 섹터 분석", "💼 내 보유종목", "🔍 종목 상세"])
    ...
    with tab1:
        ...
        tree = df.copy()
        tree["표시명"] = tree.apply(lambda x: "📌 " + x["종목명"] if x["보유종목"] else x["종목명"], axis=1)
        ...
        fig.update_layout(margin=dict(t=10, l=10, r=10, b=10), coloraxis_showscale=True, height=700)
        fig.update_traces(
            textinfo="label",
            hovertemplate="<b>%{label}</b><br>시가총액: %{value:,.0f}<br>점수: %{color}<extra></extra>"
        )
    ...
    with tab2:
        ...
        fig_sector.update_layout(height=600, yaxis=dict(categoryorder="total ascending"), coloraxis_showscale=False)
    ...
    with tab3:
        ...
        if not portfolio:
            st.warning("입력한 종목을 KRX에서 찾지 못했습니다.")
        else:
            portfolio_df = pd.DataFrame(portfolio)
            display_columns = ["종목명", "섹터", "점수", "5년수익률", "1년수익률", "3개월수익률", "거래량모멘텀", "변동성"]
            
            st.dataframe(
                portfolio_df[display_columns].sort_values("점수", ascending=False),
                use_container_width=True,
                hide_index=True
            )

            st.markdown("### 📈 보유종목 점수")
            chart_port = portfolio_df[["종목명", "점수"]].sort_values("점수")
            fig_port = px.bar(
                chart_port,
                x="점수",
                y="종목명",
    ...
    with tab4:
        ...
        c2.metric("1년 수익률", f"{selected_data['1년수익률']:.2f}%")
        c3.metric("3개월 수익률", f"{selected_data['3개월수익률']:.2f}%")
    ...
    ranking_columns = ["종목명", "섹터", "점수", "5년수익률", "1년수익률", "3개월수익률", "거래량모멘텀", "변동성"]
    ranking = df[ranking_columns].copy()
    ranking.insert(0, "순위", range(1, len(ranking) + 1))
    st.dataframe(ranking, use_container_width=True, hide_index=True)

else:
    st.info("👆 위의 **「시장 지도 분석 시작」** 버튼을 누르면 KRX 시가총액 상위 종목을 분석합니다.")
    st.markdown("""
### 📌 이 앱에서 확인할 수 있는 것
**① 시장 전체 지도**: 시가총액 기반 Treemap, 종목별 투자점수, 섹터별 자금 집중도 파악  
**② 섹터 강도**: AI/반도체, 조선/방산, 로봇, 바이오, 자동차, 2차전지, 금융 등  
**③ 내 보유종목**: 종합점수, 5년/1년/3개월 수익률, 거래량 모멘텀, 변동성  
**④ 개별 종목**: 최근 주가 차트, 추세점수, 거래량 변화, 최신 뉴스  
""")
