...
def load_universe():
    ...
    for col in required:
        if col not in df.columns:
            raise ValueError(
                f"FinanceDataReader 데이터에 {col} 컬럼이 없습니다."
            )

    df = df.copy()

    df["Marcap"] = pd.to_numeric(
        df["Marcap"],
        errors="coerce"
    )
...
def get_price_data(stock_code):
    ...
    start_date = (
        end_date -
        datetime.timedelta(days=365 * 5 + 30)
    )
...
def analyze_stock(
    ...
    if len(close) < 20:
        ...
        return {
            ...
            "시가총액": marcap,
            "거래량": volume,
            "현재가": close.iloc[-1] if len(close) else 0,
            "5년수익률": 0,
            "1년수익률": 0,
            "3개월수익률": 0,
            "거래량모멘텀": 0,
...
if st.session_state.get(
    ...
    with tab4:
        ...
        c1.metric(
            "종합점수",
            f"{selected_data['점수']}점"
        )

        c2.metric(
            "1년 수익률",
            f"{selected_data['1년수익률']:.2f}%"
        )

        c3.metric(
            "3개월 수익률",
            f"{selected_data['3개월수익률']:.2f}%"
        )

        c4.metric(
            "거래량 모멘텀",
            f"{selected_data['거래량모멘텀']:.1f}%"
        )
        ...
        c1.metric(
            "5년 수익률",
            f"{selected_data['5년수익률']:.2f}%"
        )

        c2.metric(
            "변동성",
            f"{selected_data['변동성']:.1f}%"
        )

        c3.metric(
            "추세점수",
            f"{selected_data['추세점수']} / 100"
        )
        ...
        if (
            isinstance(chart_df, pd.DataFrame)
